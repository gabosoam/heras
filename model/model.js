...
module.exports = {
    ...
    readBuy: function (callback) {
        connection.query('SELECT  * FROM v_unidadSalida ORDER BY description;', function (error, results, fields) {
            if (error) {
                console.log(error);
                callback('error en la consulta: ' + error, null);
            } else {
    ...
    readSell: function (callback) {
        connection.query('SELECT  * FROM v_modelview2 ORDER BY description;', function (error, results, fields) {
            if (error) {
                console.log(error);
                callback('error en la consulta: ' + error, null);
            } else {
    ...
    readSellOne: function (id,callback) {
        console.log(id);
        connection.query('SELECT  * FROM v_modelview3 WHERE id=? ORDER BY description;',id, function (error, results, fields) {
            if (error) {
                console.log(error);
                callback('error en la consulta: ' + error, null);
            } else {
